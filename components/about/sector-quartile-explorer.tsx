...
export function SectorQuartileExplorer() {
  ...
  return (
    <div className="border border-border/40 bg-card/25 p-4 sm:p-5 my-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-border/30 pb-3">
        <div>
          <span className="font-mono text-xs sm:text-sm uppercase tracking-wider text-foreground font-semibold block">
            Interactive Sector Quartile Atlas
          </span>
          <span className="font-mono text-[11px] sm:text-xs text-muted-foreground/80 mt-0.5 block">
            Empirical P25, Median (P50), and P75 distribution cutpoints across 13 sector cohorts
          </span>
        </div>
        <span className="font-mono text-[11px] uppercase tracking-wider text-accent border border-accent/40 bg-accent/5 px-2.5 py-0.5 shrink-0 self-start sm:self-auto font-medium">
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-3.5">
        {METRICS.map((metric) => (
          <button
            ...
            className={`px-3 py-1 font-mono text-xs uppercase tracking-wider transition-all duration-150 border ${
        ...
        <table className="w-full text-left font-mono text-xs">
          <thead>
            <tr className="border-b border-border/40 text-muted-foreground text-[11px] uppercase tracking-wider">
              <th className="py-2 pr-3 font-normal">Sector Cohort</th>
              <th className="py-2 px-2.5 text-right font-normal">Sample (N)</th>
              <th className="py-2 px-2.5 text-right font-normal text-muted-foreground/70">P25 (Bottom)</th>
              <th className="py-2 px-2.5 text-right font-semibold text-accent">P50 (Median)</th>
              <th className="py-2 px-2.5 text-right font-normal text-foreground">P75 (Top)</th>
              <th className="py-2 pl-3 min-w-[130px] font-normal">Interquartile Range (IQR)</th>
            </tr>
          </thead>
            ...
            {sectorNames.map((sectorName) => {
              ...
              return (
                <tr key={sectorName} className="hover:bg-accent/[0.02] transition-colors">
                  <td className="py-2 pr-3 text-foreground font-medium text-pretty">{sectorName}</td>
                  <td className="py-2 px-2.5 text-right text-muted-foreground/70 tabular-nums">{n}</td>
                  <td className="py-2 px-2.5 text-right text-muted-foreground/80 tabular-nums">
                    {p25.toFixed(activeMetricMeta.decimals)}
                    {activeMetricMeta.unit === "%" ? "%" : activeMetricMeta.unit === "x" ? "x" : ""}
                  </td>
                  <td className="py-2 px-2.5 text-right text-accent font-semibold tabular-nums">
                    {p50.toFixed(activeMetricMeta.decimals)}
                    {activeMetricMeta.unit === "%" ? "%" : activeMetricMeta.unit === "x" ? "x" : ""}
                  </td>
                  <td className="py-2 px-2.5 text-right text-foreground font-medium tabular-nums">
      ...
      <div className="mt-3 pt-2.5 border-t border-border/20 flex flex-col sm:flex-row sm:items-center justify-between gap-2 font-mono text-[11px] text-muted-foreground/70">
        <span>
